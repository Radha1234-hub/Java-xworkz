package com.xworkz.inheritenceapp.insect;

public class Inscet {
    public void insectDetails() {
        System.out.println("Insects are small invertebrates belonging to the class Insecta.");
        System.out.println("They have three main body segments: head, thorax, and abdomen.");
        System.out.println("Insects typically have six legs and may have wings or antennae.");
        System.out.println("Examples include butterflies, bees, ants, and beetles.");
        System.out.println("Insects play vital roles in pollination, decomposition, and the food chain.");
    }
}
