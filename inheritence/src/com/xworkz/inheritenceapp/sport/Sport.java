package com.xworkz.inheritenceapp.sport;

public class Sport {
    public void sportDetails() {

        System.out.println("Cricket is a team sport played with a bat, ball, and wickets.");
        System.out.println("It is typically played between two teams of eleven players.");
        System.out.println("The game consists of batting, bowling, and fielding.");
        System.out.println("Cricket matches can be Test, ODI, or T20 formats.");
        System.out.println("Strategy and teamwork play a crucial role in cricket.");
        System.out.println("\n");
    }
}
